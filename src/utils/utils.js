...
const getSingleCropCoeff = (numdays, croptype) => {
  // -----------------------------------------------------------------------------------------
  ...
  // -----------------------------------------------------------------------------------------
  const Lini = modeldata.cropinfo[croptype]["Lini"];
  const Ldev = modeldata.cropinfo[croptype]["Ldev"];
  const Lmid = modeldata.cropinfo[croptype]["Lmid"];
  const Llate = modeldata.cropinfo[croptype]["Llate"];
  const Kcini = modeldata.cropinfo[croptype]["Kcini"];
  const Kcmid = modeldata.cropinfo[croptype]["Kcmid"];
  const Kcend = modeldata.cropinfo[croptype]["Kcend"];
  let Kc = null;
...
const getPcpnANDPET = async (sdate, lat, lon) => {
  const pcpn = await getPcpn(sdate, lat, lon);
  const pet = await getPET(sdate, lat, lon);
  const results = pet.map((obj, i) => ({
    ...obj,
    pcpn: pcpn[i][1],
    dailyWaterDeficit: i === 0 ? 0 : null
  }));
  console.log(results);
  return results;
};

export const calcDeficit = async (sdate, lat, lon, soilCapacity) => {
  const results = await getPcpnANDPET(sdate, lat, lon);
  const TAW = getTawForPlant(soilCapacity);

  const test = results.map((obj, i) => {
    const t = {};
    const Ks = getWaterStressCoeff(obj.dailyWaterDeficit, TAW);
  });
